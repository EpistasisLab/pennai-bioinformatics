# William La Cava
# this is a benchmark experiment using hyperopt-sklearn. 
# hyper-opt sklearn is first configured to use the same hyperparameter spaces as
# available in the PMLB knowledgebase, for fair comparison to PennAI recommenders.
# for each dataset, hyperopt-sklearn is run for 1000 evaluations.
# the score of the final model is compared to the best PMLB score to determine
# success rates after each evaluation. 
import pdb
import os
import pandas as pd
import numpy as np
...
from hyperopt_setup import hype_clf

# define a comparison function that tests a recommender on the datasets, 
#using an intial knowledge base.
def run_experiment(dataset,n_recs,trial,knowledge_base,ml_p, iters):
