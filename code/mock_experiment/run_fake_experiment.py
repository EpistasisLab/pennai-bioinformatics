...
#    subset_ml = np.random.choice(ml_p['algorithm'].unique(), size= n_init)
    subset_ml = ml_p['algorithm'].unique()
    train_subset = []
    for s in subset_ml:
...
#TODO - no shuffling
#        dataset = datasets[int(datasetid/10)]
        ...
        mls, ps, scores = recommender.recommend(dataset_id=dataset,
                                        n_recs=n_recs,
#                                        dataset_mf=alocal_get_metafeatures(dataset)
                                        dataset_mf=a
                                            )
...
if __name__ == '__main__':
    ...
    parser.add_argument('-iters',action='store',dest='iters',type=int,default=1000,
                        help='Number of initial datasets to seed knowledge database')
...
#                        default='mock_experiment/sklearn-benchmark5-data-mock_experiment.tsv.gz',
                        default='mock_experiment/fake_experiment1000.tsv.gz',
                        help='Number of initial datasets to seed knowledge database')
