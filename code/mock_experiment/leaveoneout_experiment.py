...
def run_experiment(rec,dataset,n_recs,trial,knowledge_base,ml_p, iters):
    ...
    iters = np.min(iters, len(ml_p))
    print('iters set to ', iters)
    ####################################################### recs loop
    for it in np.arange(iters):
        accuracy_lookup = knowledge_base.loc[
        ...
        holdout_accuracy_lookup = knowledge_base.loc[
                knowledge_base['dataset'] == dataset].set_index(
            ['algorithm', 'parameter_hash']).loc[:, 'test_bal_accuracy'].to_dict()
        ...
        for i in np.arange(len(mls)):
            ...
            actual_score = accuracy_lookup[(ml, phash)]
            holdout_score = holdout_accuracy_lookup[(ml, phash)]
            # test_ranking = holdout_rank_lookup[(ml,phash)]
            # find all top ranking algorithms
            dataset_results = knowledge_base.loc[
                    knowledge_base['dataset'] == dataset]
            best_holdout_score = dataset_results['test_bal_accuracy'].max()
            ...
            score_delta = (best_holdout_score-holdout_score)/best_holdout_score
            ...
            results.append({'iteration':it,
                            ...
                            'score-rec':scores[0],
                            'bal_accuracy':actual_score,
                            'test_bal_accuracy':holdout_score,
                            'max_bal_accuracy':best_holdout_score,
                            # 'best_algorithm':best_algorithm,
                            # 'ranking':actual_ranking,
                            'delta_bal_accuracy':score_delta,
